...
func TestPostgresConnection(t *testing.T) {
	...
	if conn == nil || conn.Name != "testcontainer-postgres" {
		t.Fatal("Expected valid connection metadata")
	}

	if setup.DBType() != "postgres" {
		t.Errorf("Expected dbType 'postgres', got '%s'", setup.DBType())
	}
}
...
func TestPostgresLoadFixture(t *testing.T) {
	...
	// Fixtures are already loaded via WithInitScripts
...
func TestPostgresQueryCancellation(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
...
func TestPostgresSchemaOperations(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	if publicSchema.Name == "" {
		t.Fatal("Expected to find 'public' schema")
	}

	// Test getting tables
	tables, err := driver.GetTables(ctx, publicSchema)
	if err != nil {
	...
	indexes, err := driver.GetTableIndexes(ctx, "users")
	if err != nil {
		t.Fatalf("Failed to get indexes: %v", err)
	}
	// Note: indexes may be empty depending on implementation details
	// The important thing is that the query succeeded without error
	t.Logf("Found %d indexes on users table", len(indexes))
